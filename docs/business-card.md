...
> 작성: 2026-05-01 · 최종 갱신: 2026-06-10 (중소기업 전산 통합 유지보수 포지션·blue 단일 브랜드 색·한글 태그라인 반영)
...
- 우상단: **한글 태그라인 메인** `전산유지보수 · 백업·보안 / 가상 데스크톱(VDI) 기술지원` + 영문 서브 `IT Maintenance · Backup · VDI Support` — 받는 사람이 5초 안에 업종 인지
...
- 상단: `Myloket ●` (dot은 blue) + **헤드라인** `서버부터 PC까지, 회사 전산 한 곳에서 관리` (홈 히어로와 동일 메시지, 흰색 7.5pt)
- 중앙: **3 서비스 카드** (액센트 전부 `#60a5fa` blue 통일)
  - 전산통합유지보수 (IT Maintenance) — PC·서버·네트워크·백업 정기 점검과 장애 대응
  - 백업·보안 점검 (Acronis Cyber Protect) — 사고 전에 실제 복구 가능성을 확인
  - 가상 데스크톱(VDI) 기술지원 (Citrix · Omnissa Horizon) — VDI 구축·전환·장애 대응 지원
- 하단: `Citrix · Omnissa · VMware · Acronis` · **`010-3861-8079 · myloket.co.kr`** (뒷면만 보여도 전화 가능)
...
뒷면 3 서비스가 빡빡하게 느껴지면 `.practices` 안의 3개 중 일부만 남김. 영업 담당자별 강조 서비스 다르게 가능. **단 가상 데스크톱(VDI) 기술지원 블록은 가능한 유지** (마이로켓 차별화 포인트).
...
- **받는 사람 우선** — 앞면 태그라인 한글 메인 (중소기업 대표·총무가 5초 안에 업종 인지), 뒷면 헤드라인은 홈 히어로와 동일 메시지
