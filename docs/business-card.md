...
> 작성: 2026-05-01 · 최종 갱신: 2026-07-12 (VDI 구축·기술지원 전면 포지션 반영 — 서비스 순서 VDI 우선, 태그라인 교체, Vinchin 파트너 추가)
...
### 앞면 (Front)
- **좌측 blue 단색 라인** (`#1d4ed8`) — 홈페이지 2색 체계 통일 (2026-06-10)
- 좌상단: `Myloket ●` wordmark (dot은 blue)
- 우상단: **한글 태그라인 메인** `가상 데스크톱(VDI) 구축·기술지원 / 전산 통합 유지보수 · 백업·복구검증` + 영문 서브 `VDI Support · IT Maintenance · Backup` — 받는 사람이 5초 안에 업종 인지
- 중앙: **이름 한글 (18pt) → Je Hyunwoo → 대표 · 수석 기술지원 엔지니어 / CEO · Principal Engineer**
- 좌하단: 연락처 stack (M / T / E)
- 우하단: ㈜마이로켓 · Myloket Inc. · myloket.co.kr (blue)

### 뒷면 (Back)
- **다크 배경 (#0f172a)** — 플랫 (코너 글로우 제거 — 홈페이지 '장식 제거' 원칙 + CMYK 리스크 제거)
- 상단: `Myloket ●` (dot은 blue) + **헤드라인** `VDI 구축·기술지원부터 전산 통합 유지보수까지` (홈 히어로와 동일 메시지, 흰색 7.5pt)
- 중앙: **3 서비스 카드** (액센트 전부 `#60a5fa` blue 통일)
  - 가상 데스크톱(VDI) 구축·기술지원 (Citrix · Omnissa Horizon) — VDI 구축·전환·장애 대응 지원
  - 전산 통합 유지보수 (IT Maintenance) — PC·서버·네트워크·백업 정기 점검과 장애 대응
  - 백업·복구검증 (Acronis · Vinchin) — 사고 전에 실제 복구 가능성을 확인
- 하단: `Citrix · Omnissa · VMware · Acronis · Vinchin` · **`010-3861-8079 · myloket.co.kr`** (뒷면만 보여도 전화 가능)
...
- 위험 색상 (2026-06-10 blue 단일 체계 전환으로 리스크 축소됨):
  - `#1d4ed8` (blue-700, 메인) → 채도 손실 가능성 (CMYK 재현성은 purple보다 양호)
  - `#60a5fa` / `#93c5fd` (뒷면 라이트 블루) → 채도 손실 가능성
...
5. 종이: 250~300g/m² 아트지
6. 메인 컬러: blue #1d4ed8 (브랜드 시그니처 — 좌측 라인·dot) — 채도 손실 최소화 부탁
```
...
뒷면 3 서비스가 빡빡하게 느껴지면 `.practices` 안의 3개 중 일부만 남김. 영업 담당자별 강조 서비스 다르게 가능. **단 가상 데스크톱(VDI) 구축·기술지원 블록은 반드시 유지** (VDI 우선 포지션의 핵심·마이로켓 차별화 포인트).
...
- **받는 사람 우선** — 앞면 태그라인 한글 메인·VDI 우선 (받는 사람이 5초 안에 업종 인지), 뒷면 헤드라인은 홈 히어로와 동일 메시지
- **blue 단일 액센트** — 홈페이지 2색 체계(blue-700 + amber CTA)와 통일. 서비스별 색 구분 폐기 (2026-06-10)
- **용어 규칙** — 'VDI' 단독 표기 대신 '가상 데스크톱(VDI)' 병기 (1차 타깃 이해도)
