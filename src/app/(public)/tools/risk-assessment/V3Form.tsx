...
/**
 * 구버전(v3) 진단 폼. ?ver=3 으로만 접근하므로 page.tsx에서 dynamic import 한다.
 * 한 모듈에 두면 questions.v3(약 12.7KB)가 기본 경로 번들에 함께 실린다.
 */
...
export default function V3Form() {
              ...
              <div className="flex justify-between mt-3">
                {["정보", ...SECTIONS.map((s) => s.title.split("/")[0].slice(0, 3))].map((label, i) => (
                  <div key={i} className="flex flex-col items-center gap-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-2xs font-semibold transition-colors ${i + 1 < currentStepNum ? "bg-blue-600 text-white" : i + 1 === currentStepNum ? "bg-blue-600 text-white ring-2 ring-blue-200" : "bg-gray-200 text-gray-600"}`}>
                    ...
                    <span className={`text-2xs ${i + 1 <= currentStepNum ? "text-blue-600 font-medium" : "text-gray-600"}`}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <form noValidate onSubmit={handleLeadSubmit} className="space-y-5">
            <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label htmlFor="r3-name" className="block text-sm font-medium text-gray-700 mb-1.5">이름</label>
                  <input id="r3-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="홍길동"
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
                </div>
                <div>
