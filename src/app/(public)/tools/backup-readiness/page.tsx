...
const INTEREST_AREAS = [
  { value: "backup-policy", label: "백업 정책 수립" },
  { value: "ransomware", label: "랜섬웨어 대응" },
  { value: "dr-failover", label: "DR·페일오버" },
  { value: "acronis-poc", label: "Acronis Cyber Protect PoC" },
  { value: "compliance", label: "사이버복원력 컴플라이언스" },
  { value: "consulting", label: "전반 컨설팅" },
] as const;

export default function BackupReadinessDiagnosisPage() {
  ...
  const [interestAreas, setInterestAreas] = useState<string[]>(["backup-policy"]);
  ...
  function handleLeadSubmit(e: React.FormEvent) {
    ...
    if (!email.trim()) found["br-email"] = "이메일을 입력해주세요.";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) found["br-email"] = "이메일 형식을 확인해주세요.";
    if (!organizationName.trim()) found["br-organizationName"] = "기관·회사명을 입력해주세요.";
    if (!consent) found["br-consent"] = "개인정보 처리 동의가 필요합니다.";
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 2. Run diagnosis
      const runRes = await fetch("/api/tools/backup-readiness/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    return (
      <div key={q.id} className="space-y-1.5">
        ...
        <select
          ...
          className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
        >
          ...
          <form noValidate onSubmit={handleLeadSubmit} className="space-y-5">
            <div className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="br-organizationName" className="block text-sm font-medium text-gray-700 mb-1.5">
                    기관명 <span className="text-red-600" aria-hidden="true">*</span>
                  </label>
                  <input
                    id="br-organizationName"
                    type="text"
                    value={organizationName}
                    onChange={(e) => setOrganizationName(e.target.value)}
                    required
                    aria-invalid={!!fieldErrors["br-organizationName"]}
                    aria-describedby={fieldErrors["br-organizationName"] ? "br-organizationName-error" : undefined}
                    className={`w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 ${fieldErrors["br-organizationName"] ? "border-red-400 bg-red-50/50" : ""}`}
                    placeholder="예: 한국OO공단"
                  />
                {fieldErrors["br-organizationName"] && (
                  <p id="br-organizationName-error" className="mt-1.5 text-xs text-red-700">{fieldErrors["br-organizationName"]}</p>
                )}
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="br-email" className="block text-sm font-medium text-gray-700 mb-1.5">
                  ...
                  <input
                    ...
                    required
                    aria-invalid={!!fieldErrors["br-email"]}
                    aria-describedby={fieldErrors["br-email"] ? "br-email-error" : undefined}
                    className={`w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 ${fieldErrors["br-email"] ? "border-red-400 bg-red-50/50" : ""}`}
                    placeholder="your@email.com"
                  />
                {fieldErrors["br-email"] && (
                  <p id="br-email-error" className="mt-1.5 text-xs text-red-700">{fieldErrors["br-email"]}</p>
                )}
                </div>
                <div>
              ...
              <div>
                ...
                <textarea id="br-message"
                  ...
                  placeholder="현재 백업 환경의 고민이나 상담받고 싶은 주제를 자유롭게 적어주세요."
                />
              </div>
              ...
              <label className="flex items-start gap-3 p-3 rounded-lg bg-gray-50 border border-gray-200 cursor-pointer hover:bg-emerald-50 hover:border-emerald-200 transition-colors">
                <input
                  ...
                  aria-invalid={!!fieldErrors["br-consent"]}
                  aria-describedby={fieldErrors["br-consent"] ? "br-consent-error" : undefined}
              ...
              {fieldErrors["br-consent"] && (
                <p id="br-consent-error" className="text-xs text-red-700">{fieldErrors["br-consent"]}</p>
              )}
            </div>
